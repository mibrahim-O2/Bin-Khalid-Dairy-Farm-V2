...
export function BillEditorClient({
  customerId,
  bill,
  customer,
  products,
  ...
  const ratesByProductId = useMemo(() => {
    const map: Record<string, CustomerRate> = {};
    for (const rate of rates) map[rate.productId] = rate;
    return map;
  }, [rates]);

  // Local editable draft — initialized once from the server-fetched bill,
  // no live listener to clobber in-progress edits with (there isn't one
  // anymore — see src/lib/db/README.md on why onSnapshot went away).
  ...
  async function handleSaveDraft(): Promise<boolean> {
    ...
    setSaving(true);
    setError(null);
    const result = await updateDraftBill({
      billId: bill.id,
      startDate,
      endDate,
      lineItems,
  ...
  async function handleFinalize() {
    ...
    const result = await finalizeBill({ billId: bill.id });
    setFinalizing(false);
    if (!result.ok) {
      setError(result.error);
      return;
