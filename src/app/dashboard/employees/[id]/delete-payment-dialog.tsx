...
export function DeletePaymentDialog({ paymentId }: { paymentId: string }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setDeleting(true);
    setError(null);
    const result = await deleteEmployeePayment({ paymentId, reason: reason.trim() || undefined });
    setDeleting(false);
    if (!result.ok) {
    ...
    <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (next) setError(null); }}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This permanently removes this advance/payment from the ledger and database. This
            cannot be undone.
          </DialogDescription>
        </DialogHeader>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-2">
            ...
            <Textarea id="delete-payment-reason" value={reason} onChange={(e) => setReason(e.target.value)} rows={2} />
          </div>
