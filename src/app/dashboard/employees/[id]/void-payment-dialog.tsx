...
import { voidEmployeePayment } from "../actions";
...
export function VoidPaymentDialog({ paymentId }: { paymentId: string }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSaving(true);
    setError(null);
    const result = await voidEmployeePayment({ paymentId, reason: reason.trim() });
    setSaving(false);
    if (!result.ok) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This never deletes or edits the original payment — it records a reversing credit and
            marks the payment void, permanently.
          </DialogDescription>
        </DialogHeader>
