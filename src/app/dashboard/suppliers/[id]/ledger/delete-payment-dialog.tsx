...
/** Owner-only — see deleteSupplierPayment's doc comment for why this replaced Void. */
export function DeletePaymentDialog({ paymentId }: { paymentId: string }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSaving(true);
    setError(null);
    const result = await deleteSupplierPayment({ paymentId, reason: reason.trim() || undefined });
    setSaving(false);
    if (!result.ok) {
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            restoring any purchases it settled back to unpaid/partially-paid. This cannot be
            undone.
          </DialogDescription>
        </DialogHeader>
