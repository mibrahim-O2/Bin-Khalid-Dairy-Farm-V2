...
export function StatementsCard({ supplierId }: { supplierId: string }) {
  const { user } = useCurrentUser();
  const [statements, setStatements] = useState<SupplierStatement[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "supplierStatements"),
    ...
    return onSnapshot(
      q,
      (snapshot) => {
        setStatements(snapshot.docs.map((d) => ({ id: d.id, ...d.data() }) as SupplierStatement));
      },
