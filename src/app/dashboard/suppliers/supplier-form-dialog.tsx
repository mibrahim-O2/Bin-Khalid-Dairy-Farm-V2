...
export function SupplierFormDialog({ supplier, trigger, onCreated }: SupplierFormDialogProps) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setError(null);
    if (supplier) {
      const result = await updateSupplier({ supplierId: supplier.id, name, phone, whatsappNumber, address });
      setSaving(false);
      if (!result.ok) {
      ...
      const result = await createSupplier({ name, phone, whatsappNumber, address });
      setSaving(false);
      if (!result.ok) {
        setError(result.error);
        return;
